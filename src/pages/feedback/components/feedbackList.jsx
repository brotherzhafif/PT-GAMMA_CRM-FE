  ...
  return (
    <Card className="flex flex-col bg-white dark:bg-card rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm h-full">
        ...
        {paginatedData.length === 0 ? (
          ...
          <div className="flex flex-col gap-3 p-3 sm:p-4">
            {paginatedData.map((item) => {
              ...
              const sourceInfo = channelIcons[item.source] || {
                icon: Globe,
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex min-w-0 flex-col gap-0.5">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-1">
                            <div className={` rounded ${sourceInfo.color}`}>
                    ...
                    <div className="flex flex-shrink-0 flex-wrap items-center gap-1.5">
                      <Badge
                        variant="outline"
                        className={`text-[9px] px-1.5 py-0 font-medium ${sentimentColors[item.sentiment]}`}
                      >
                        {item.sentiment}
                      </Badge>
                      {/* <Badge
                        className={`text-[9px] px-1.5 py-0.5 rounded-full font-semibold border-none ${statusColors[item.status]}`}
                      >
                        {item.status}
                      </Badge>
                      <Badge
                        variant="secondary"
                        className="text-[9px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20 px-1.5 py-0 rounded border-none hover:bg-emerald-50 dark:hover:bg-emerald-950/20"
                      >
                        {item.category}
                      </Badge> */}
                    </div>
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-300 line-clamp-2 leading-relaxed mt-3">
                    {item.comment}
                  </p>

                  {/* Card Footer area — hanya tampil kalau ada replies */}
                  {item.replies.length > 0 && (
                    <div className="flex items-center justify-end gap-2 mt-1 pt-2 border-t border-slate-100 dark:border-gray-900">
      ...
      {data.length > ITEMS_PER_PAGE && (
        <CardFooter className="px-4 py-0 border-t border-gray-200 dark:border-gray-800 flex-shrink-0 bg-white dark:bg-card flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
