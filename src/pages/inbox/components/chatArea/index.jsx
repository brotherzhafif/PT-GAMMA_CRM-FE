  ...
  const processedMessages = useMemo(() => {
    return messages.reduce(
      (acc, msg) => {
        ...
        let nextHandler = acc.currentHandler;

        if (msg.sender === "agent") {
          if (msg.senderType === "human" && acc.currentHandler === "ai") {
            showEscalation = true;
            nextHandler = "human";
          } else if (
            msg.senderType === "ai" &&
            acc.currentHandler === "human"
          ) {
            showBotReturn = true;
            nextHandler = "ai";
          } else {
            nextHandler = msg.senderType;
          }
        }
          ...
          <div className="px-4 py-4">
            ...
            <div className="space-y-1">
              {processedMessages.map((msg) => (
                <div key={msg.id}>
                  {msg.showEscalation && <StatusDivider type="human" />}

                  {msg.showBotReturn && <StatusDivider type="bot" />}
